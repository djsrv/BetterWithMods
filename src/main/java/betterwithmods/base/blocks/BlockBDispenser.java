...
import betterwithmods.base.blocks.tile.TileEntityBlockDispenser;
import betterwithmods.base.client.BWCreativeTabs;
import betterwithmods.base.util.InvUtils;
import betterwithmods.modules.automation.features.SawFakePlayer;
            ...
            if (!world.getBlockState(pos).getValue(TRIGGERED)) {
                ...
                if (!world.isAirBlock(check) && !world.getBlockState(check).getBlock().isReplaceable(world, check) && world.getBlockState(check).getBlockHardness(world, check) != -1.0F) {
                    ...
                    if (stacks.isEmpty() && state.getBlock().canSilkHarvest(world, check, state, null))
                        stacks.add(state.getBlock().getPickBlock(state, null, world, check, SawFakePlayer.player));
                    else if (!stacks.isEmpty()) {
                        for (ItemStack stack : stacks) {
                            if (ItemStack.areItemsEqual(stack, state.getBlock().getPickBlock(state, null, world, check, SawFakePlayer.player))) {
                                stacks.remove(stack);
                                stacks.add(state.getBlock().getPickBlock(state, null, world, check, SawFakePlayer.player));
                                break;
                            }
