package betterwithmods.modules.utilities.blocks;
...
public class BlockLens extends BWMBlock {
    ...
    public static final int RANGE = Redstone.lensRange;
    ...
    public void updateTick(World world, BlockPos pos, IBlockState state, Random rand) {
        ...
        if (!isLightDetector) {
            ...
            if (isLit(world, pos) && (world.isAirBlock(offset) || world.getBlockState(offset).getBlock() == Redstone.LENS_SOURCE)) {
                for (int i = 1; i < RANGE; i++) {
                    BlockPos bPos = pos.offset(dir, i);
                    IBlockState lightState = Redstone.LENS_SOURCE.getDefaultState();
                    if (world.isAirBlock(bPos)) {
                        ...
                        if (entity.size() > 0) {
                            ...
                            break;
                        } else if (world.getBlockState(bPos).getBlock() == Redstone.LENS_SOURCE && world.getBlockState(bPos).getValue(DirUtils.FACING).ordinal() < expectedFacing.ordinal()) {
                            ...
                            if (world.getBlockState(bPos).getBlock() != Redstone.LENS_SOURCE || (world.getBlockState(bPos).getBlock() == Redstone.LENS_SOURCE && world.getBlockState(bPos).getValue(DirUtils.FACING).ordinal() < expectedFacing.ordinal()))
                                world.setBlockState(bPos, lightState.withProperty(DirUtils.FACING, expectedFacing).withProperty(BlockInvisibleLight.SUNLIGHT, sunlight));
                            break;
                        }
                        ...
                        if (world.getBlockState(dPos).getBlock() != Redstone.LENS_SOURCE || (world.getBlockState(dPos).getBlock() == Redstone.LENS_SOURCE && world.getBlockState(dPos).getValue(DirUtils.FACING).ordinal() <= expectedFacing.ordinal())) {
                            if (world.getBlockState(dPos).getBlock() == Redstone.LENS_SOURCE && world.getBlockState(dPos).getValue(BlockInvisibleLight.SUNLIGHT))
                ...
                for (int i = 1; i < RANGE; i++) {
                    ...
                    if (world.getBlockState(bPos).getBlock() == Redstone.LENS_SOURCE) {
    ...
    private boolean isLightFromSun(World world, BlockPos pos) {
        ...
        if (world.isAirBlock(offset) && world.canBlockSeeSky(offset))
            return true;
        else if (world.getBlockState(offset).getBlock() == Redstone.LENS_SOURCE && world.getBlockState(offset).getValue(BlockInvisibleLight.SUNLIGHT))
            return true;
        return false;
    }
    ...
    private void cleanupLightToFacing(World world, BlockPos pos, EnumFacing facing) {
        ...
        for (int i = 1; i < RANGE; i++) {
            ...
            if (world.getBlockState(offset).getBlock() == Redstone.LENS_SOURCE) {
