...
public class BlockChime extends BWMBlock implements IMultiVariants {
    ...
    public boolean onBlockActivated(World world, BlockPos pos, IBlockState state, EntityPlayer player, EnumHand hand, @Nullable ItemStack heldItem, EnumFacing side, float hitX, float hitY, float hitZ) {
        if (world.isRemote)
            ...
            if (!state.getValue(ACTIVE)) {
                ...
                world.playSound(null, pos, state.getMaterial() == Material.IRON ? BWMSounds.METALCHIME : BWMSounds.WOODCHIME, SoundCategory.BLOCKS, 0.4F, 1.0F);
    ...
    public void updateTick(World world, BlockPos pos, IBlockState state, Random rand) {
        ...
        if (storm)
            world.playSound(null, pos, state.getMaterial() == Material.IRON ? BWMSounds.METALCHIME : BWMSounds.WOODCHIME, SoundCategory.BLOCKS, 0.25F + (rand.nextFloat() - rand.nextFloat() * 0.1F), 1.0F);
        world.scheduleBlockUpdate(pos, this, tickRate(world), 5);
    }
    ...
    public void onEntityCollidedWithBlock(World world, BlockPos pos, IBlockState state, Entity entity) {
        if (!state.getValue(ACTIVE)) {
            ...
            for (EnumFacing facing : EnumFacing.VALUES)
                world.notifyNeighborsOfStateChange(pos.offset(facing), this);
            world.playSound(null, pos, state.getMaterial() == Material.IRON ? BWMSounds.METALCHIME : BWMSounds.WOODCHIME, SoundCategory.BLOCKS, 1.0F, 1.0F);
        }
    }
