package betterwithmods.modules.utilities.entity;
...
public class EntityMiningCharge extends Entity {
    ...
    /**
     * returns if this entities triggers Block.onEntityWalking on the blocks they walk on. used for spiders and wolves to
    ...
    /**
     * Called to update the entities's position/logic.
     */
    @Override
    ...
    private void explodeBlock(World world, BlockPos pos) {
        ...
        if (resistance < 100) {
            ...
            if (state.getBlock().canDropFromExplosion(explosion)) {
                if (state.getBlock() == Blocks.COBBLESTONE) //TODO: Possible charge registry to blast a blocks into an item.
    ...
    /**
     * (abstract) Protected helper method to write subclass entities data to NBT.
     */
    @Override
    ...
    /**
     * (abstract) Protected helper method to read subclass entities data from NBT.
     */
    @Override
