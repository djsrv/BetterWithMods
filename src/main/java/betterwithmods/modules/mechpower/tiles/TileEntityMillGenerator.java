...
public abstract class TileEntityMillGenerator extends TileEntity implements ITickable, IMechanicalPower {
    //Every generator will take up a single blocks with no extended bounding box
    public int radius;
    ...
    public void update() {
        if (this.runningState != 0) {
            ...
            if (this.getWorld().rand.nextInt(100) == 0)
                this.getWorld().playSound(null, pos, BWMSounds.WOODCREAK, SoundCategory.BLOCKS, 0.25F, getWorld().rand.nextFloat() * 0.25F + 0.25F);
        }
