package betterwithmods.modules.mechpower.tiles;

import betterwithmods.api.block.IAxle;
import betterwithmods.api.block.IMechanicalBlock;
import betterwithmods.api.tile.IColor;
...
public class TileEntityWindmillVertical extends TileEntityMillGenerator implements IColor {
    ...
    public boolean isValid() {
        ...
        boolean valid = true;
        if (getWorld().getBlockState(pos).getBlock() != null && getWorld().getBlockState(pos).getBlock() == Generators.WINDMILL) {
            for (int i = -3; i < 4; i++) {
                if (i == 0)
                    continue;
    ...
    private void invalidateWindmill() {
        this.getWorld().setBlockState(this.pos, Generators.WINDMILL.getDefaultState());
        for (int i = -3; i < 4; i++) {
            BlockPos pos = this.pos.add(0, i, 0);
            if (getWorld().getBlockState(pos).getBlock() instanceof BlockAxle)
                this.getWorld().setBlockState(pos, Transfer.AXLE.getDefaultState());
        }
        if (!this.getWorld().isRemote)
            InvUtils.ejectStackWithOffset(getWorld(), pos, new ItemStack(Generators.WINDMILL, 1, 2));
        this.getWorld().setBlockState(this.pos, Transfer.AXLE.getDefaultState());
    }
