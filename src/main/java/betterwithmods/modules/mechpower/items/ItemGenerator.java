...
public class ItemGenerator extends Item implements IMultiLocations {
    ...
    private boolean isHorizontalDeviceValid(EntityPlayer player, World world, BlockPos pos, int meta, EnumFacing.Axis axis) {
        ...
        if (meta == 1 && validateWaterwheel(world, pos, axis)) {
            if (axis != EnumFacing.Axis.Y) {
                world.setBlockState(pos, ((BlockWaterwheel) Generators.WATERWHEEL).getAxisState(axis));
                valid = true;
            }
        } else if (meta == 1) {
            if (world.isRemote)
        ...
        if (meta == 0 && validateWindmill(world, pos, axis)) {
            if (axis != EnumFacing.Axis.Y) {
                world.setBlockState(pos, ((BlockWindmill) Generators.WINDMILL).getAxisState(axis));
                valid = true;
            }
        } else if (meta == 0) {
            if (world.isRemote)
    ...
    private boolean isVerticalWindmillValid(EntityPlayer player, World world, BlockPos pos, float flY) {
        ...
        if (checkForSupportingAxles(world, target)) {
            if (validateArea(player, world, target)) {
                world.setBlockState(target, Generators.WINDMILL.getDefaultState().withProperty(BlockMillGenerator.AXIS, EnumFacing.Axis.Y));//BlockPowerSource.setProxies(world, x, yPos, z);
                valid = true;
    ...
    private boolean checkForSupportingAxles(World world, BlockPos pos) {
        for (int i = -3; i <= 3; i++) {
            ...
            if (!(targetBlock instanceof BlockAxle)) return false;
