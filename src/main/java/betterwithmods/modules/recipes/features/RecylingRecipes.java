...
//            }
        ...
        if (ModuleLoader.isFeatureEnabled(HCOre.class)) {
            InvUtils.getOreNames("nuggetIron").stream().findAny().ifPresent( stack -> {
                addStokedCrucibleRecipe(new ItemStack(stack.getItem(), 3), new ItemStack[]{new ItemStack(Items.BUCKET)});
            });
        } else {
