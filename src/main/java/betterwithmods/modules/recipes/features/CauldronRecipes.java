...
import betterwithmods.base.items.ItemBark;
...
public class CauldronRecipes extends Feature {
    ...
    public void cookableFoods() {
        Map<ItemStack, ItemStack> furnace = net.minecraft.item.crafting.FurnaceRecipes.instance().getSmeltingList();
        for (ItemStack input : furnace.keySet()) {
            if (input != null) {
                if (input.getItem() instanceof ItemFood && input.getItem() != Items.BREAD) {
                    ItemStack output = net.minecraft.item.crafting.FurnaceRecipes.instance().getSmeltingResult(input);
                    if (output != null) {
