package betterwithmods.modules.hardcore.blocks;
...
public class BlockStump extends Block implements IMultiVariants {
    ...
    public static IBlockState getStump(IBlockState log) {
        ...
        return HCStumping.STUMP.getDefaultState().withProperty(BlockPlanks.VARIANT, variant);
    }
    ...
    public List<ItemStack> getDrops(IBlockAccess world, BlockPos pos, IBlockState state, int fortune) {
        ...
        if (state.getProperties().containsKey(BlockPlanks.VARIANT)) {
            ret.add(new ItemStack(Carpentry.BARK, 1, state.getValue(BlockPlanks.VARIANT).getMetadata()));
        }
        for (int i = 0; i < 6; ++i) {
            ret.add(new ItemStack(Carpentry.SAW_DUST));
        }
        return ret;
    }
    ...
    public void onBlockDestroyedByExplosion(World worldIn, BlockPos pos, Explosion explosionIn) {
        for (int i = 0; i < 4; ++i) {
            spawnAsEntity(worldIn, pos, new ItemStack(Carpentry.SAW_DUST));
        }
    }
