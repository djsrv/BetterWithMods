...
import betterwithmods.api.crafting.heat.BWMHeatSource;
...
public abstract class TileEntityCookingPot extends TileEntityVisibleInventory {
    ...
    public void update() {
        ...
        if (this.getWorld().getBlockState(this.pos).getBlock() instanceof BlockCookingPot) {
            ...
            if (!block.isMechanicalOn(this.getWorld(), this.pos)) {
                ...
                if (getWorld().getBlockState(pos).getValue(BlockCookingPot.ISACTIVE)) {
    ...
    private boolean captureDroppedItems() {
        ...
        if (items.size() > 0) {
            ...
            for (EntityItem item : items) {
                flag = InvUtils.putDropInInventoryAllSlots(inventory, item) || flag;
    ...
    protected boolean containsExplosives() {
        if(ModuleLoader.isFeatureEnabled(Explosives.class)) {
            return containsItem(Explosives.BLASTING_OIL);
        }
        return containsItem(Item.getItemFromBlock(Blocks.TNT)) || containsItem(Items.GUNPOWDER);// || containsItem(BWMItems.MATERIAL, 16); //HELLFIRE
    }
