...
public class BlockAnchor extends BlockDirectional implements ITurnable {
            ...
            if (heldItem.getItem() instanceof ItemBlock) {
                ...
                if (block instanceof BlockRope) {
                    if (!world.isRemote) {
                        ...
                        if (blockDown instanceof BlockRope) {
                            BlockRope.placeRopeUnder((BlockRope)blockDown,heldItem, world, down, player);
                            ...
                            world.playSound(null, down, block.getSoundType(block.getDefaultState(), world, null, null).getPlaceSound(), SoundCategory.BLOCKS, 1, 1);
    ...
    private boolean retractRope(World world, BlockPos pos, EntityPlayer player,BlockRope rope) {
        for (int i = pos.getY() - 1; i >= 0; i--) {
            BlockPos pos2 = new BlockPos(pos.getX(), i, pos.getZ());
            if (world.getBlockState(pos2).getBlock() != rope && world.getBlockState(pos2.up()).getBlock() == rope) {
                if (!player.capabilities.isCreativeMode)
                    addRopeToInv(world, pos, player,rope);
                return world.setBlockToAir(pos2.up());
            } else if (world.getBlockState(pos2).getBlock() != rope)
                return false;
        }
        return false;
    }

    private void addRopeToInv(World world, BlockPos pos, EntityPlayer player,BlockRope rope) {
        ItemStack item = new ItemStack(rope);
        ...
        if (player.inventory.addItemStackToInventory(item))
            ...
            InvUtils.ejectStackWithOffset(world, pos, item);
    }
    ...
    private boolean isRope(IBlockAccess world, BlockPos origin, EnumFacing facing) {
        ...
        return world.getBlockState(pos).getBlock() instanceof BlockRope;
    }
