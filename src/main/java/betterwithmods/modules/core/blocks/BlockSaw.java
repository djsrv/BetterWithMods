...
import betterwithmods.base.util.BWMDamageSource;
...
public class BlockSaw extends BWMBlock implements IMechanicalBlock {
    ...
    public void neighborChanged(IBlockState state, World world, BlockPos pos, Block block) {
        //TODO not required
        if (block == Transfer.AXLE || block == HandCrank.HAND_CRANK)
            world.scheduleBlockUpdate(pos, this, tickRate(world), 5);
        else
    ...
    public void onEntityCollidedWithBlock(World world, BlockPos pos, IBlockState state, Entity entity) {
        if (isMechanicalOn(world, pos) && entity instanceof EntityLivingBase) {
            ...
            DamageSource source = BWMDamageSource.saw;
            int damage = 4;
            ...
            for (int i = 0; i < 3; i++) {
                ...
                if(ModuleLoader.isFeatureEnabled(ChoppingBlock.class)) {
                    if (block == ChoppingBlock.CHOPPING_BLOCK && blockState.getValue(BlockAesthetic.blockType).getMeta() < 2) {
                        source = BWMDamageSource.choppingBlock;
                        damage *= 3;
                        if (blockState.getValue(BlockAesthetic.blockType).getMeta() == 0 && unobstructed)
                            world.setBlockState(pos2, ChoppingBlock.CHOPPING_BLOCK.getDefaultState());
                        break;
                    }
                }
    ...
    private void performLastHit(Entity entity, float damage) {
        entity.attackEntityFrom(DamageSource.causePlayerDamage(Carpentry.player), damage);
    }
...
//            InvUtils.ejectBrokenItems(world, pos, new ResourceLocation(BWMod.MODID, "blocks/saw"));
        /*
