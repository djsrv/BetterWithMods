...
public class BlockBellows extends BWMBlock implements IMechanicalBlock {
    ...
    private void stokeFlames(World world, BlockPos pos) {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            if (target == Blocks.FIRE || target == Stoking.STOKED_FLAME)
    ...
    private void stokeFire(World world, BlockPos pos) {
        ...
        if (world.getBlockState(down).getBlock() == Stoking.HIBACHI) {
            int flag = (world.getBlockState(pos).getBlock() == Stoking.STOKED_FLAME) ? 4 : 3;
            world.setBlockState(pos, Stoking.STOKED_FLAME.getDefaultState(), flag);
        } else
