...
public final class BlockMechMachines extends BWMBlock implements IMechanicalBlock, ITileEntityProvider, IMultiVariants {
    ...
    private static boolean dropsMill;
    ...
    public String[] getVariants() {
        return new String[]{
                "facing=up,ison=false,machinetype=mill",
    ...
    private void breakPulley(World world, BlockPos pos) {
        if (MechanicalBlocks.dropsPulley)
    ...
    public void breakHopper(World world, BlockPos pos) {
        if (MechanicalBlocks.dropsHopper)
    ...
    private void breakTurntable(World world, BlockPos pos) {
        if (MechanicalBlocks.dropsTurntable)
    ...
    public IBlockState getStateFromMeta(int meta) {
        return this.getDefaultState().withProperty(MACHINETYPE, EnumType.byMeta(meta)).withProperty(ISACTIVE, (meta & 1) == 1);
    }

    @Override
    public int getMetaFromState(IBlockState state) {
        int type = state.getValue(MACHINETYPE).getMeta();
