...
import betterwithmods.modules.core.blocks.behavior.DispenserBehaviorBlock;
...
public class BlockBDispenser extends BlockDispenser implements ITurnable, IMultiVariants {
    public static final RegistryDefaulted<Item, IBehaviorDispenseItem> BLOCK_DISPENSER_REGISTRY = new RegistryDefaulted<>(new DispenserBehaviorBlock());
            ...
            if (!world.getBlockState(pos).getValue(TRIGGERED)) {
                ...
                if (!world.isAirBlock(check) && !world.getBlockState(check).getBlock().isReplaceable(world, check) && world.getBlockState(check).getBlockHardness(world, check) != -1.0F) {
                    ...
                    if (stacks.isEmpty() && state.getBlock().canSilkHarvest(world, check, state, null))
                        stacks.add(state.getBlock().getPickBlock(state, null, world, check, DispenserBlock.player));
                    else if (!stacks.isEmpty()) {
                        for (ItemStack stack : stacks) {
                            if (ItemStack.areItemsEqual(stack, state.getBlock().getPickBlock(state, null, world, check, DispenserBlock.player))) {
                                stacks.remove(stack);
                                stacks.add(state.getBlock().getPickBlock(state, null, world, check, DispenserBlock.player));
                                break;
                            }
                ...
                if (!creatures.isEmpty() && creatures.size() > 0) {
                    for (EntityLivingBase creature : creatures) {
                        if (creature instanceof EntityWolf) {
                            tile.addStackToInventory(new ItemStack(DispenserBlock.WOLF), check);
